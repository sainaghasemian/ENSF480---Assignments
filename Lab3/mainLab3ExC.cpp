...
//modification of global function print():
template<class Key, class Datum> 
void print(LookupTable<Key, Datum>& lt);


//void try_to_find(LookupTable& lt, int key);
//modification of global function try_to_find():
template<class Key, class Datum> 
void try_to_find(LookupTable<Key, Datum>& lt, int key);
...
template<class Key, class Datum> void print(LookupTable<Key, Datum>& lt)
{
  if (lt.size() == 0)
    cout << "  Table is EMPTY.\n";
...
template<class Key, class Datum> void try_to_find(LookupTable<Key, Datum>& lt, int key)
{
  lt.find(key);
  if (lt.cursor_ok())
    cout << "\nFound key:"  << lt;
