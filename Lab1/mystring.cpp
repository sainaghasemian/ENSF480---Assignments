...
Mystring& Mystring::append(const Mystring other) //Copy constructor is called because an object of class Mystring is passed by value to a function
{
  char *tmp = new char [lengthM + other.lengthM + 1];
  lengthM += other.lengthM;
  strcpy(tmp, charsM);
  strcat(tmp, other.charsM);
  delete []charsM; //Destructor called
  charsM = tmp;
